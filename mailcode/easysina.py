...
def getSeedemail():
    get_seed_sql = '''
        ...
        where 
            ge.domain_name in ('163.com', '126.com', 'sina.com', 'sina.cn')
        order by
            ge.domain_name;
    '''
    try:
...
def insertData(data):
    insert_sql = '''
        INSERT INTO `headerinfo` 
            (`re_time`, `uid`, `mid`, `box_status`, `from_addree`, `seed_email`, `domain_name`, `sender`, `subject`) 
        VALUES 
            ('%s', %s, %s, %s, '%s', '%s', '%s', '%s', '%s');
    '''
    try:
            ...
            for num in string.split(data[0]):
                try:
                    ...
                    if reply != "":
                        ...
                        header_info = []
                        # print "------------------------"
                        data = tuple([send_time, user_id, message_id, box_status, from_address, email_address, domain_name, sender, subject])
                        insertData(data)
                    else:
                        print "This mail subject: %s doesn't sended from epcare!" % (getmailheader(msg.get('Subject', '')))
                        continue
                except Exception, e:
