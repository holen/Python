...
def insertData(data):
    insert_sql = '''
        INSERT INTO `headerinfo1` 
            (`re_time`, `uid`, `mid`, `box_status`, `from_ip`, `from_addree`, `seed_email`, `domain_name`, `sender`, `subject`, `box_id`) 
        VALUES 
                ...
                # if int(num) > 0:
                    try:
                        ...
                        if reply != "":
                            ...
                            # print message_id
                            if domain_name == '163.com' or domain_name == '126.com':
                                received = msg.get('Received', '')
                                from_ip = re.search(r'\[(.*)\]', received).group(1)
                            elif domain_name == 'sina.com' or domain_name == 'sina.cn':
                                received = msg.get('X-Mda-Received', '')
                                from_ip = re.search(r'\[<(.*)>\]', received).group(1)
                            else:
                                received = msg.get_all('Received', '')
                                from_ip = re.search(r'\[(.*)\]', received[1]).group(1)
                            ...
                            # print sender
                            # print "------------------------"
                            data = tuple([send_time, user_id, message_id, box_status, from_ip, from_address, email_address, domain_name, sender, subject, num])
                            insertData(data)
                            # print data
                        else:
...
if __name__ == '__main__':
    args = sys.argv;
    if len(args) == 1 :
        domain_name = """'163.com','126.com','sina.com','sina.cn','sohu.com'"""
        datas = getSeedemail(domain_name)
    elif args[1] == 'sohu':
        domain_name = """'sohu.com'"""
        datas = getSeedemail(domain_name)
    elif args[1] == 'sina':
        domain_name = """'sina.com','sina.cn'"""
        datas = getSeedemail(domain_name)
    else:
        domain_name = """'163.com','126.com'"""
        datas = getSeedemail(domain_name)
    for data in datas:
